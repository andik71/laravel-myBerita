                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form enctype="multipart/form-data" method="POST"
                            action="{{ url('administrator/article/'.$article->id.'/update') }}">
                            @csrf
                            @method('put')
                            <div class="row mb-3">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <div class="input-group">
                                        <textarea type="text" name="content" rows="6" class="form-control" id="inputText">{{ $article->content}}</textarea>
                                        <script>
                                    ...
                                    <div class="input-group">
                                        ...
                                        <select class="form-select" name="author_id" aria-selected="Select Author">
                                            ...
                                            @foreach ($author as $row)
                                                <option {{ $article->author_id == $row->id ? 'selected' : '' }} value="{{ $row->id }}">{{ $row->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="input-group">
                                        ...
                                        <select class="form-select" name="category_id" aria-selected="Select Category">
                                            ...
                                            @foreach ($category as $row)
                                                <option {{ $article->category_id == $row->id ? 'selected' : '' }} value="{{ $row->id }}">{{ $row->category }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
